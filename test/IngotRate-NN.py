...
X_train, X_test, y_train, y_test = train_test_split(source_data[:, :14], source_data[:, 14], test_size=0.05,
                                                    random_state=0)
...
class LinearModel(torch.nn.Module):
    ...
    def __init__(self):
        ...
        # 输入为14维，in_features = 14
        # 使用权重矩阵将in_feature的维度空间映射到out_feature的维度空间
        self.nn1 = torch.nn.Linear(14, 8)
        self.nn2 = torch.nn.Linear(8, 6)
        self.nn3 = torch.nn.Linear(6, 4)
        self.nn4 = torch.nn.Linear(4, 1)
        self.activate = torch.nn.ReLU()
        self.sigmoid = torch.nn.Sigmoid()

    def forward(self, x):
        x = self.activate(self.nn1(x))
        x = self.activate(self.nn2(x))
        x = self.activate(self.nn3(x))
        x = self.sigmoid(self.nn4(x))
        return x
